  ...
  <div class="page-container">
        ...
        <el-table-column label="进度" width="120">
          <template #default="{ row }">
            <div style="display: flex; align-items: center; gap: 8px;">
              <el-progress
                :percentage="row.groupCount > 0 ? Math.round((row.checkedGroupCount || 0) / row.groupCount * 100) : 0"
                :status="row.checkedGroupCount >= row.groupCount ? 'success' : ''"
                :stroke-width="8"
                style="flex: 1;"
              />
              <span style="font-size: 12px; color: #666; white-space: nowrap;">
                {{ row.checkedGroupCount || 0 }}/{{ row.groupCount || 0 }}
              </span>
            </div>
          </template>
        </el-table-column>
    ...
    <el-card v-if="selectedCheck" class="detail-card">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center">
          <div>
            ...
            <span style="margin-left: 20px; color: #999">
              进度: {{ checkedGroupCount }}/{{ checkGroups.length }}
              <span v-if="differenceGroupCount > 0" style="color: #f56c6c"> (差异: {{ differenceGroupCount }})</span>
            </span>
          </div>
      ...
      <el-table :data="checkGroups" v-loading="groupsLoading" border>
        ...
        <el-table-column label="数量(件)" width="80">
          <template #default="{ row }">
        ...
        <el-table-column label="实盘数量" width="140">
          <template #default="{ row }">
            <span v-if="row.status > 0" :class="{ 'text-danger': row.difference !== null && row.difference.compareTo(0) !== 0 }">{{ row.actualQuantity }}</span>
        ...
        <el-table-column label="差异" width="80">
          <template #default="{ row }">
            <span v-if="row.status > 0" :class="{ 'text-danger': row.difference !== null && row.difference.compareTo(0) > 0, 'text-warning': row.difference !== null && row.difference.compareTo(0) < 0 }">
              {{ row.difference !== null ? (row.difference.compareTo(0) > 0 ? '+' : '') + row.difference : '-' }}
            </span>
          ...
          <template #default="{ row }">
            <div class="action-buttons">
              <template v-if="row.status === 0">
                <el-button link type="primary" size="small" @click="handleCheckGroup(row)">确认盘点</el-button>
              </template>
...
import {
  getStockCheckList, getStockCheckGroups, createStockCheck,
  checkStockGroup, completeStockCheck, adjustStockCheckGroup
} from '@/api/stockCheck'
...
// 计算已盘点分组数量
const checkedGroupCount = computed(() => {
  return checkGroups.value.filter(item => item.status > 0).length
})

// 计算有差异的分组数量
const differenceGroupCount = computed(() => {
  return checkGroups.value.filter(item => item.status === 2).length
})
...
const handleSelectCheck = async (row) => {
  ...
  try {
    const res = await getStockCheckGroups(row.id)
    checkGroups.value = (res.data || []).map(group => ({
      ...group,
      inputQuantity: group.systemQuantity,
      inputDifferenceReason: group.differenceReason || ''
    }))
  } finally {
...
const handleCheckGroup = async (row) => {
  ...
  // 调用新的盘点接口
  await checkStockGroup(selectedCheck.value.id, {
    groupId: row.id,
    actualQuantity: row.inputQuantity,
    remarks: row.inputDifferenceReason || ''
  })
...
const getDifference = (row) => {
  const input = row.inputQuantity || 0
...
const confirmAdjust = async () => {
  ...
  await adjustStockCheckGroup(currentGroup.value.id, adjustReason.value)
