            ...
            <div style="display: flex; justify-content: space-between; align-items: center">
              <div>
                ...
                <span style="margin-left: 20px; color: #999">
                  进度: {{ checkedGroupCount }}/{{ checkItems.length }}
...
import {
  getStockCheckList, getStockCheckItemsGrouped, createStockCheck,
  checkStockCheckBatch, completeStockCheck, adjustStockCheckItem
} from '@/api/stockCheck'
...
const checkedGroupCount = computed(() => {
  return checkItems.value.filter(item => item.status > 0).length
})

// 计算有差异的分组数量
const differenceGroupCount = computed(() => {
  return checkItems.value.filter(item => item.status === 2).length
})
...
const handleSelectCheck = async (row) => {
  ...
  try {
    const res = await getStockCheckItemsGrouped(row.id)
    checkItems.value = (res.data || []).map(group => ({
      ...group,
...
const handleCheckItem = async (row) => {
  ...
  // 使用批量盘点接口
  await checkStockCheckBatch(selectedCheck.value.id, {
    itemIds: row.itemIds,
...
const confirmAdjust = async () => {
  ...
  // 调整组内第一个有差异的明细
  const diffItem = currentGroup.value.itemIds[0]
  await adjustStockCheckItem(diffItem, adjustReason.value)
