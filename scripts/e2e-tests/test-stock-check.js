...
async function testStockCheck() {
  ...
  try {
    ...
    await page.waitForTimeout(1000);
    // 1. 检查左右布局
    const leftCol = await page.locator('.el-col-8, .el-col-6, .el-col-4').count() > 0;
    const rightCol = await page.locator('.el-col-16, .el-col-18, .el-col-20').count() > 0;
    logStep('左右分栏布局', leftCol > 0 && rightCol > 0);
    (leftCol > 0 && rightCol > 0) ? result.addPass() : result.addFail('布局');
    // 2. 检查盘点任务列表
    ...
    if (dialogVisible > 0) {
      ...
      const scopeSelect = await page.locator('.el-dialog:visible .el-select').count() > 0;
      logStep('表单字段完整', dateInput > 0 && scopeSelect > 0);
      (dateInput > 0 && scopeSelect > 0) ? result.addPass() : result.addFail('表单字段');
      // 关闭对话框
      await page.click('.el-dialog:visible button:has-text("取消")');
      await page.waitForTimeout(300);
    }
    // 6. 测试选择盘点任务
    const taskRows = await page.locator('.el-table tbody tr');
